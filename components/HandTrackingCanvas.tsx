...
import { Results, VisualGuideType } from '../types';
import { motion } from 'framer-motion';
import { ArrowUp, ArrowRight, RotateCw, RefreshCcw, MoveVertical, MoveHorizontal } from 'lucide-react';
...
// --- VISUAL GUIDANCE OVERLAY COMPONENT ---
const GuidanceOverlay: React.FC<{ type: VisualGuideType }> = ({ type }) => {
  if (!type || type === 'none') return null;

  return (
    <div className="absolute inset-0 z-30 pointer-events-none flex items-center justify-center overflow-hidden">
      {/* Motion: Forward (simulated as "Outwards" / Scale Up) */}
      {type === 'motion_forward' && (
        <div className="flex flex-col items-center">
            <motion.div
                initial={{ opacity: 0, scale: 0.5, y: 50 }}
                animate={{ opacity: [0, 1, 0], scale: [0.5, 1.2, 1.5], y: [50, 0, -20] }}
                transition={{ duration: 2, repeat: Infinity, ease: "easeOut" }}
                className="flex flex-col items-center"
            >
                 <div className="w-20 h-20 border-4 border-blue-500 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(59,130,246,0.5)]">
                     <ArrowUp className="w-10 h-10 text-blue-400" strokeWidth={3} />
                 </div>
                 <span className="mt-4 text-xl font-bold text-blue-400 uppercase tracking-widest bg-black/50 px-4 py-1 rounded-full backdrop-blur-md">Move Forward</span>
            </motion.div>
        </div>
      )}
      ...
      {type === 'motion_up' && (
        <div className="flex flex-col items-center">
            <motion.div
                ...
                animate={{ opacity: [0, 1, 0], y: [100, -50, -100] }}
                transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}
            >
                <div className="flex flex-col items-center gap-2">
                    <ArrowUp className="w-16 h-16 text-emerald-400 drop-shadow-lg" strokeWidth={3} />
                    <ArrowUp className="w-16 h-16 text-emerald-400/50" strokeWidth={3} />
                    <span className="text-xl font-bold text-emerald-400 uppercase tracking-widest bg-black/50 px-4 py-1 rounded-full backdrop-blur-md">Lift Up</span>
                </div>
            </motion.div>
        </div>
      )}
      ...
      {type === 'motion_circle' && (
        <div className="flex flex-col items-center">
            <motion.div
                ...
                className="relative w-64 h-64 rounded-full border-4 border-dashed border-amber-500/50 flex items-center justify-center"
            >
                 <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-amber-500 text-black p-2 rounded-full">
                    <RotateCw className="w-6 h-6" />
                 </div>
            </motion.div>
            <div className="absolute mt-80 bg-black/50 px-4 py-1 rounded-full backdrop-blur-md">
                 <span className="text-lg font-bold text-amber-400 uppercase tracking-widest">Circular Motion</span>
            </div>
        </div>
      )}
      ...
      {type === 'motion_shake' && (
        <div className="flex flex-col items-center">
             ...
             >
                <MoveHorizontal className="w-16 h-16 text-red-400 drop-shadow-lg" strokeWidth={3} />
             </motion.div>
             <span className="mt-4 text-xl font-bold text-red-400 uppercase tracking-widest bg-black/50 px-4 py-1 rounded-full backdrop-blur-md">Shake</span>
        </div>
      )}
      ...
      {type === 'motion_nod' && (
        <div className="flex flex-col items-center">
             <motion.div
                ...
                transition={{ duration: 1, repeat: Infinity, ease: "easeInOut" }}
             >
                <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center border-2 border-white backdrop-blur-sm">
                    <MoveVertical className="w-8 h-8 text-white" />
                </div>
             </motion.div>
             <span className="mt-4 text-xl font-bold text-white uppercase tracking-widest bg-black/50 px-4 py-1 rounded-full backdrop-blur-md">Nod Fist</span>
        </div>
      )}
    </div>
  );
...
const HandTrackingCanvas = forwardRef<HandTrackingRef, HandTrackingCanvasProps>(({ onHandsDetected, show3DView = true, visualGuide = 'none' }, ref) => {
  ...
  return (
    <div className="relative w-full h-full bg-black/80 rounded-3xl overflow-hidden">
      ...
      {/* Dynamic Visual Guide Overlay */}
      <GuidanceOverlay type={visualGuide} />
    </div>
  );
});
