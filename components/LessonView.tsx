...
const LessonView: React.FC<Props> = ({ lesson, user, onBack, onComplete, hasNext, onNext }) => {
  ...
  const handleVerify = async () => {
    ...
    if (imageBase64) {
      ...
      if (result.isCorrect) {
        ...
        setVerifyDuration(5000);
        if (hasNext) {
           setTimeout(() => onComplete(updatedUser), 1500);
        } else {
  ...
  return (
    <div className="relative w-full h-full min-h-screen bg-black text-white overflow-hidden flex flex-col">
      {/* Background Ambience */}
      <div className="absolute inset-0 bg-gradient-to-br from-zinc-900 via-black to-zinc-900 z-0 pointer-events-none" />
      ...
      {/* Main Workspace */}
      <main className="flex-1 w-full max-w-[1920px] mx-auto p-8 pt-4 flex gap-6 overflow-hidden z-10 relative">
        
        {/* Left: Reference Portal (Image Only) */}
        <section className="w-1/3 flex flex-col gap-6 relative group">
           <div className="flex-1 glass-panel rounded-[32px] overflow-hidden relative border border-white/5 shadow-2xl bg-gradient-to-b from-zinc-800/20 to-zinc-900/50">
              {/* Content */}
               <div className="w-full h-full flex items-center justify-center p-12">
                   <img 
                        src={lesson.imageUrl} 
                        alt={lesson.letter}
                        className="w-full h-full object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)] opacity-90 dark-invert"
                   />
               </div>

              {/* Instructions Overlay */}
              <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-black via-black/80 to-transparent">
                  <p className="text-lg text-zinc-200 font-medium leading-relaxed mb-1">
                      {lesson.description}
                  </p>
                  {lesson.instruction && (
        ...
        {/* Right: Camera Portal */}
        <section className="flex-1 relative rounded-[32px] overflow-hidden shadow-2xl border border-white/10 bg-black">
             <HandTrackingCanvas 
                ref={canvasRef} 
             ...
             <AnimatePresence>
                {feedback && (
                    ...
                    >
                        <div className={`glass-panel-heavy px-8 py-5 rounded-2xl border flex items-center gap-6 shadow-[0_8px_32px_rgba(0,0,0,0.5)] ${feedback.isCorrect ? 'border-green-500/30 shadow-green-900/20' : 'border-orange-500/30 shadow-orange-900/20'}`}>
             ...
             <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20">
                <button
                    ...
                    className={`group px-8 py-4 rounded-full font-bold flex items-center gap-3 transition-all ${
                        !handDetected ? 'bg-white/5 text-zinc-600 cursor-not-allowed' : 
